...
class Network:
    ...
    def init(self, env, args):
        ...
        self.model.add(tf.keras.layers.Dense(args.actions**2 + args.actions*(args.actions-1)))
        self.model.compile(
            optimizer='rmsprop',
            loss='mse',
            metrics=['mse'],
        ...
        self.target.add(tf.keras.layers.Dense(args.actions**2 + args.actions*(args.actions-1)))
        self.target.compile(
            optimizer='rmsprop',
            loss='mse',
            metrics=['mse'],
    ...
    def save(self, experimental=False, value=0):
        # Save network with current timestamp
        name = './networks/{}.model'.format(round(time.time()))
        if experimental:
            name += '.experimental.{}'.format(value)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gamma", default=0.93, type=float, help="Discounting factor.")

    parser.add_argument("--buffer", default=1024*16, type=int, help="Replay buffer size.")
    parser.add_argument("--actions", default=5, type=int, help="To how many buckets discretize actions.")
    ...
    parser.add_argument("--test_each", default=20, type=int, help="How often to reset target network (in steps).")
    ...
    rs = collections.deque(maxlen=40)
    ...
    while episode < args.episodes:
        ...
        if step % args.test_each == 0:
            ...
            print('Reward {} -- {}'.format(R, sum(rs)/len(rs)))

            if sum(rs)/len(rs) > 220:
                print('Let\'s take the risk')
                network.save(True, sum(rs)/len(rs))
